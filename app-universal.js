(() => {
  ...
  function ensureMainNavigation() {
    ...
    // V1.0.88: nu mai reconstruim meniul dacă are deja aceeași ordine; schimbăm doar active/href.
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.querySelectorAll('.brand-version').forEach(el => { el.textContent = 'v' + (cfg.version || '1.0.88'); });
  }, { once: true });
  ...
  // V1.0.88 - meniu stabil: pe desktop rămâne extins, iar click pe meniu nu îl restrânge automat.
  document.addEventListener('DOMContentLoaded', () => {
    const shell = document.querySelector('.app-shell');
    if (!shell) return;
